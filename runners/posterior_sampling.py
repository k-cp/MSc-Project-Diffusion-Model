...
class PosteriorRunner:
    ...
    def dps_sample(self, low_res_measurement, zeta=1.0):
        ...
        self.log(
            f"Starting DPS reverse sampling: {len(timesteps)} steps, "
        ...
        for i in tqdm(reversed(timesteps), total=len(timesteps), desc="DPS sampling"):
            ...
            x0_hat = (x - torch.sqrt(1.0 - alpha_bar_t) * noise_pred) / torch.sqrt(
                alpha_bar_t
            )
            x0_hat = torch.clamp(x0_hat, -1.0, 1.0)

            y_hat = self.fluid_downsample_operator(
                x0_hat, scale_factor=self.args.scale_factor
            )
            ...
            with torch.no_grad():
                ...
                if i > 0:
                    prev_idx = max(i - 1, 0)
                    variance = beta_t * (1.0 - self.alphas_bar[prev_idx]) / (
                        1.0 - alpha_bar_t
                    )
